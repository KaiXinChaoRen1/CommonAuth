...
public class IndexController {
    ...
    public Result login(@RequestBody LoginVo loginVo) {
        //1.根据username查询是否有用户
        MySystemUser MySystemUser = MySystemUserService.getUserInfoByUserName(loginVo.getUsername());
        ...
        if (!MySystemUser.getPassword().equals(md5Password)) {
        ...
        String token = JwtHelper.createToken(MySystemUser.getId(), MySystemUser.getUsername());
        //返回
    ...
    public Result info(HttpServletRequest request) {
        ...
        Map<String, Object> map = MySystemUserService.getUserInfo(username);
        return Result.ok(map);
    }
